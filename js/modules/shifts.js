// ════════════════════════════════════════════════════════════════
// ROBERT OS - SHIFTS.JS v1.7.5 (STABLE EXPORT)
// ════════════════════════════════════════════════════════════════
...
export const actions = {
    'start': async () => {
        try {
            ...
            console.error('Shift start fail:', err);
...
export async function loadActive() {
    ...
    try {
        ...
        console.error('Shifts load fail:', err);
        widget.innerHTML = '<div class="text-red-500 text-[10px] p-4 uppercase italic">Klaida</div>';
    }
}
    ...
    container.innerHTML = `
        ...
        ${active ? `
            <div class="p-4 bg-teal-500/5 border border-teal-500/20 rounded-2xl">
                <p class="text-teal-500 font-black text-xs italic tracking-widest uppercase">Aktyvi Pamaina</p>
                <p class="text-[10px] text-white/40 mt-2 uppercase font-mono">${new Date(active.start_time).toLocaleTimeString('lt-LT')}</p>
            </div>
        ` : `
            <button data-action="shifts:start" class="w-full py-4 bg-teal-500 text-black font-black text-[10px] rounded-2xl italic tracking-widest hover:bg-teal-400 transition-all">PRADĖTI DARBĄ</button>
        `}
    `;
}
