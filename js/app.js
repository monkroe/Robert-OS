...
async function refreshProjection() {
    ...
    if (nw) {
        ...
        container.innerHTML = `<div class="text-center mb-8"><p class="label-tiny">Net Worth</p><p class="text-4xl font-black">$${parseFloat(nw.total_net_worth).toLocaleString()}</p></div>`;
        if (goals) container.innerHTML += goals.map(g => `<div class="glass-card p-6 flex justify-between items-center"><div><p class="label-tiny">Target</p><p class="font-black">$${(g.target_net_worth/1000).toFixed(0)}K</p></div><div class="text-right"><p class="text-2xl font-black text-primary-500">${g.months_to_goal ? (g.months_to_goal/12).toFixed(1)+' m.' : 'STALL'}</p></div></div>`).join('');
    }
}

async function refreshVault() {
    const { data } = await db.from('investment_portfolio_view').select('*');
    if (data) getEl('asset-list').innerHTML = data.map(a => `<div class="glass-card p-5 flex justify-between items-center"><span class="font-bold uppercase text-xs">${a.symbol}</span><span class="font-black text-primary-500">$${parseFloat(a.market_value).toLocaleString()}</span></div>`).join('');
}

async function refreshAudit() {
    const { data } = await db.from('finance_transactions').select('*').order('date',{ascending:false}).limit(15);
    if (data) getEl('audit-list').innerHTML = data.map(t => `<div class="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between text-[10px] font-bold"><span class="opacity-40 uppercase">${new Date(t.date).toLocaleDateString()}</span><span class="${t.direction==='in'?'text-primary-500':'text-red-400'} uppercase">$${t.amount}</span></div>`).join('');
}

function openOdoModal() { getEl('odo-title').innerText = activeShiftId ? "Pabaigos rida" : "Pradžios rida"; getEl('odo-input').value = ""; getEl('odo-modal').classList.remove('hidden'); }
function closeOdoModal() { getEl('odo-modal').classList.add('hidden'); }
async function confirmShiftAction() {
    const odo = getEl('odo-input').value; if (!odo) return alert("Įveskite ridą");
